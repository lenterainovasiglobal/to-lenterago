...
window.render = function () {
  ...
  root.innerHTML = progs
    .map((p, idx) => {
      ...
      // COMPACT UPDATE: p-5 -> p-3, text-xl -> text-lg
      return `
        <div class="prog-item ${isActive ? "active" : ""}" data-prog-idx="${idx}" id="prog-item-${idx}">
          <div class="p-3 flex items-center cursor-pointer font-extrabold transition-all duration-300 ${
            isActive
              ? "justify-center text-lg text-sky-600 bg-sky-50/50"
...
function renderDetail(p, dataList, allProgs) {
  ...
  // COMPACT UPDATE: pb-6 -> pb-2, px-5 -> px-3, mb-8 -> mb-3
  return `
    <div class="pb-2">
      <div class="px-3 mb-3">
      ...
      <div class="pkg-grid-scroll px-3">
        ${subset
          ...
          .map((r) => {
            ...
            // COMPACT UPDATE:
            // 1. Title: mb-3 -> mb-1, text-lg -> text-base, min-h-[56px] -> min-h-[40px]
            // 2. Info Box: text-sm -> text-xs, mb-3 -> mb-2, py-2 -> py-1
            // 3. Price Area: pt-4 -> pt-2
            // 4. Button: mt-3 -> mt-2, py-3 -> py-1.5
            return `
              <div class="pkg-card">

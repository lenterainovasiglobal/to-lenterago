...
document.getElementById('regForm').addEventListener('submit', async (e) => {
  ...
  try {
    ...
    window.location.href = "/daftar/sukses";
...
async function init() {
  ...
  const urlPkg = urlParams.get('paket');
  const urlRef = urlParams.get('ref');
  const urlPromo = urlParams.get('promo');

  if(urlPkg) selectedPkg.code = urlPkg;
  if(urlRef) sessionStorage.setItem("to_ref_code", urlRef);
  if(urlPromo) sessionStorage.setItem("to_promo_code", urlPromo);
  ...
  const rS = sessionStorage.getItem("to_ref_code");
  const pS = sessionStorage.getItem("to_promo_code");
  if (rS) referralInput.value = rS.toUpperCase();
  if (pS) promoInput.value = pS.toUpperCase();
